...
function courseplay:turn(vehicle, dt)
	...
	local allowedAngle						= 5; -- Used for changing back into reverse if the vehicle and tool angle difference are below that.
	...
	if not vehicle.cp.courseWorkWidth then
		courseplay:calculateWorkWidth(vehicle, noDraw)
			...
			for index, turnTarget in ipairs(vehicle.cp.turnTargets) do
				if index < #vehicle.cp.turnTargets then
					...
					if turnTarget.turnReverse then
						...
						if nextTurnTarget.reverseWhenPosible then
	...
	if vehicle.cp.turnStage > 0 then
		...
		if vehicle.cp.turnStage == 1 then
			...
			----------------------------------------------------------
				...
				--- If not wheeled implement, then do the short turns.
					...
					if neededSpace < turnInfo.headlandHeight or isHarvester or not turnInfo.turnOnField then
		...
		elseif vehicle.cp.turnStage == 2 then
			if newTarget then
				...
				if newTarget.reverseWhenPosible and reversingWorkTool and reversingWorkTool.cp.realTurningNode then
					-- Get the angles of the vehicle and tool
					local _, vehicleRot, _ = getWorldRotation(vehicle.cp.DirectionNode);
					vehicleRot = deg(vehicleRot) + 180;
					local _, workToolRot, _ = getWorldRotation(reversingWorkTool.cp.realTurningNode);
					workToolRot = deg(workToolRot) + 180;

					-- Get the angle difference
					local angleDifference = abs(vehicleRot - workToolRot);

					-- If the angle diff is less than the allowed angle, then goto the first reverse wp
					courseplay:debug(("%s:(Turn) Reverse when %.2f <= %.2f"):format(nameNum(vehicle), angleDifference, allowedAngle), 14);
					if angleDifference <= allowedAngle then
						for i = vehicle.cp.curTurnIndex, #vehicle.cp.turnTargets, 1 do
							if vehicle.cp.turnTargets[i].turnReverse then
								courseplay:debug(("%s:(Turn) Changing to reverse since %.2f < %.2f"):format(nameNum(vehicle), angleDifference, allowedAngle), 14);
								vehicle.cp.curTurnIndex = i;
								return;
							end;
						end;
						courseplay:debug(("%s:(Turn) No reverse point found."):format(nameNum(vehicle), angleDifference, allowedAngle), 14);
					end;
				end;
			else
		...
		elseif vehicle.cp.turnStage == 3 then
			...
			if lowerImplements then
				...
				courseplay:setWaypointIndex(vehicle, vehicle.cp.waypointIndex + 4);
...
function courseplay:generateTurnTypeWideTurn(vehicle, turnInfo)
	...
	center1.x,_,center1.z = localToWorld(vehicle.cp.DirectionNode, turnInfo.turnRadius * turnInfo.direction, 0, -turnInfo.reverseOffset);
	center2.x,_,center2.z = localToWorld(turnInfo.targetNode, turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
	...
	startDir.x,_,startDir.z = localToWorld(vehicle.cp.DirectionNode, 0, 0, -turnInfo.reverseOffset);
	stopDir.x,_,stopDir.z = localToWorld(turnInfo.targetNode, 0, 0, (turnInfo.zOffset) * -1 + turnInfo.reverseOffset);
	...
	if not canTurnOnHeadland and not vehicle.cp.aiTurnNoBackward and turnInfo.turnOnField then
		...
		posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, turnInfo.reverseOffset + directionNodeToTurnNodeLength + turnInfo.wpChangeDistance);
		courseplay:addTurnTarget(vehicle, posX, posZ);
...
function courseplay:generateTurnTypeWideTurnWithAvoidance(vehicle, turnInfo)
	...
	if turnInfo.targetDeltaZ > 0 then
		...
		center.x,_,center.z = localToWorld(vehicle.cp.DirectionNode, turnInfo.turnRadius * turnInfo.direction, 0, -turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(vehicle.cp.DirectionNode, 0, 0, -turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(vehicle.cp.DirectionNode, turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.turnRadius - turnInfo.reverseOffset);
		...
		center.x,_,center.z = localToWorld(vehicle.cp.DirectionNode, (vehicle.cp.courseWorkWidth - turnInfo.turnRadius - turnInfo.turnDiameter) * turnInfo.direction, 0, turnInfo.turnDiameter - turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(vehicle.cp.DirectionNode, (vehicle.cp.courseWorkWidth - turnInfo.turnRadius - turnInfo.turnDiameter) * turnInfo.direction, 0, turnInfo.turnRadius - turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(vehicle.cp.DirectionNode, (vehicle.cp.courseWorkWidth - turnInfo.turnDiameter) * turnInfo.direction, 0, turnInfo.turnDiameter - turnInfo.reverseOffset);
		courseplay:generateTurnCircle(vehicle, center, startDir, stopDir, turnInfo.turnRadius, turnInfo.direction * -1, true);

		--- Generate the third turn circles
		center.x,_,center.z = localToWorld(turnInfo.targetNode, turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(turnInfo.targetNode, turnInfo.turnDiameter * turnInfo.direction, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(turnInfo.targetNode, 0, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
		courseplay:generateTurnCircle(vehicle, center, startDir, stopDir, turnInfo.turnRadius, turnInfo.direction, true);
	...
	----------------------------------------------------------
		...
		center.x,_,center.z = localToWorld(vehicle.cp.DirectionNode, turnInfo.turnRadius * turnInfo.direction, 0, -turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(vehicle.cp.DirectionNode, 0, 0, -turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(vehicle.cp.DirectionNode, turnInfo.turnDiameter * turnInfo.direction, 0, -turnInfo.reverseOffset);
		courseplay:generateTurnCircle(vehicle, center, startDir, stopDir, turnInfo.turnRadius, turnInfo.direction, true);

		--- Generate the second turn circles
		center.x,_,center.z = localToWorld(turnInfo.targetNode, (vehicle.cp.courseWorkWidth - turnInfo.turnDiameter - turnInfo.turnRadius) * turnInfo.direction, 0, turnInfo.zOffset * -1 - turnInfo.turnDiameter + turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(turnInfo.targetNode, (vehicle.cp.courseWorkWidth - turnInfo.turnDiameter) * turnInfo.direction, 0, turnInfo.zOffset * -1 - turnInfo.turnDiameter + turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(turnInfo.targetNode, (vehicle.cp.courseWorkWidth - turnInfo.turnDiameter - turnInfo.turnRadius) * turnInfo.direction, 0, turnInfo.zOffset * -1 - turnInfo.turnRadius + turnInfo.reverseOffset);
		courseplay:generateTurnCircle(vehicle, center, startDir, stopDir, turnInfo.turnRadius, turnInfo.direction * -1, true);

		--- Generate the third turn circles
		center.x,_,center.z = localToWorld(turnInfo.targetNode, turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
		startDir.x,_,startDir.z = localToWorld(turnInfo.targetNode, turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.zOffset * -1 - turnInfo.turnRadius + turnInfo.reverseOffset);
		stopDir.x,_,stopDir.z = localToWorld(turnInfo.targetNode, 0, 0, turnInfo.zOffset * -1 + turnInfo.reverseOffset);
		courseplay:generateTurnCircle(vehicle, center, startDir, stopDir, turnInfo.turnRadius, turnInfo.direction, true);
	end;
	...
	if not canTurnOnHeadland and not vehicle.cp.aiTurnNoBackward and turnInfo.turnOnField then
		...
		if turnInfo.reverseOffset - 3 > 0 then
			fromPoint.x, _, fromPoint.z = localToWorld(turnInfo.targetNode, 0, 0, turnInfo.reverseOffset - 3);
			toPoint.x, _, toPoint.z = localToWorld(turnInfo.targetNode, 0, 0, 0);
			courseplay:generateTurnStraitPoints(vehicle, fromPoint, toPoint, true, directionNodeToTurnNodeLength + extraMoveBack + turnInfo.reverseWPChangeDistance);
		else
			posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, 0);
			local revPosX, _, revPosZ = localToWorld(turnInfo.targetNode, 0, 0, -(directionNodeToTurnNodeLength + extraMoveBack + turnInfo.reverseWPChangeDistance));
			courseplay:addTurnTarget(vehicle, posX, posZ, false, true, true, revPosX, revPosZ);
...
function courseplay:generateTurnTypeQuestionmarkTurn(vehicle, turnInfo)
	...
	local centerHeightOffset = (turnInfo.zOffset + targetOffsetZ) * -1;
	...
	if doNormalTurn then
		...
		if turnInfo.haveHeadlands and newZOffset < turnInfo.directionNodeToTurnNodeLength * 0.5 then
			...
			local sectionLength = (directionNodeToTurnNodeLength + extraDistance + turnInfo.wpChangeDistance + 6) / 3;
			fromPoint.x, _, fromPoint.z = localToWorld(turnInfo.targetNode, 0, 0, newZOffset);
			toPoint.x, _, toPoint.z = localToWorld(turnInfo.targetNode, 0, 0, newZOffset + (sectionLength * 1));
			courseplay:generateTurnStraitPoints(vehicle, fromPoint, toPoint);
			fromPoint.x, _, fromPoint.z = localToWorld(turnInfo.targetNode, 0, 0, newZOffset + (sectionLength * 1));
			toPoint.x, _, toPoint.z = localToWorld(turnInfo.targetNode, 0, 0, newZOffset + (sectionLength * 3));
			courseplay:generateTurnStraitPoints(vehicle, fromPoint, toPoint, nil, nil, nil, true);
			--posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, newZOffset + directionNodeToTurnNodeLength + extraDistance + turnInfo.wpChangeDistance + 6);
			--courseplay:addTurnTarget(vehicle, posX, posZ, nil, nil, nil, nil, nil, nil, true);
		...
		if not canTurnOnHeadland then
			...
			--- Reverse back
			fromPoint.x, _, fromPoint.z = localToWorld(turnInfo.targetNode, turnInfo.targetDeltaX, 0, 3);
		...
		--- The last turn circle is less than 3m behind the lane start, so we have to reverse back.
			...
			--- Extra WP 1 - Move a bit more forward
			posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, centerHeightOffset + directionNodeToTurnNodeLength + extraDistance + turnInfo.wpChangeDistance + 6);
			courseplay:addTurnTarget(vehicle, posX, posZ);
...
function courseplay:generateTurnTypeForward3PointTurn(vehicle, turnInfo)
	...
	courseplay:debug(("%s:(Turn) centerOffset=%s, centerHeight=%s"):format(nameNum(vehicle), tostring(turnInfo.centerOffset), tostring(sideB), tostring(sideC), tostring(turnInfo.centerHeight)), 14);

	--- Get the 2 circle center cordinate
	center1.x,_,center1.z = localToWorld(turnInfo.targetNode, turnInfo.targetDeltaX - turnInfo.turnRadius * turnInfo.direction, 0, turnInfo.targetDeltaZ - turnInfo.zOffset - 1);
	center2.x,_,center2.z = localToWorld(turnInfo.targetNode, turnInfo.turnRadius * turnInfo.direction * -1, 0, turnInfo.targetDeltaZ - turnInfo.zOffset - 1 - turnInfo.centerHeight);

	--- Generate first turn circle (Forward)
	startDir.x,_,startDir.z = localToWorld(turnInfo.targetNode, turnInfo.targetDeltaX, 0, turnInfo.targetDeltaZ - turnInfo.zOffset - 1);
	...
	local zPossition = turnInfo.targetDeltaZ - turnInfo.zOffset - 1 - turnInfo.centerHeight;
	...
	posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, zPossition - 2);
	courseplay:addTurnTarget(vehicle, posX, posZ, false, false, true);
	posX, _, posZ = localToWorld(turnInfo.targetNode, 0, 0, zPossition - (turnInfo.reverseWPChangeDistance * 1.5));
	courseplay:addTurnTarget(vehicle, posX, posZ, false, false, true);
...
function courseplay:generateTurnStraitPoints(vehicle, fromPoint, toPoint, reverse, turnEnd, secondaryReverseDistance, reverseWhenPosible)
	local endTurn = false;
	local wpDistance = 3;
	...
	courseplay:addTurnTarget(vehicle, fromPoint.x, fromPoint.z, false, nil, reverse, nil, nil, nil, reverseWhenPosible);
	...
	if numPointsNeeded > 0 then
		...
		for i=1, numPointsNeeded do
			...
			courseplay:addTurnTarget(vehicle, posX, posZ ,false, nil, reverse, nil, nil, nil, reverseWhenPosible);
		end;
	end;
	...
	courseplay:addTurnTarget(vehicle, posX, posZ, false, endTurn, reverse, revPosX, revPosZ, nil, reverseWhenPosible);
...
function courseplay:addTurnTarget(vehicle, posX, posZ, useSmoothTurn, turnEnd, turnReverse, revPosX, revPosZ, dontPrint, reverseWhenPosible)
	local target = {};
	target.posX 			  = posX;
	target.posZ 			  = posZ;
	...
	target.reverseWhenPosible = reverseWhenPosible;
	table.insert(vehicle.cp.turnTargets, target);

	if not dontPrint then
		courseplay:debug(("%s:(Turn:addTurnTarget) posX=%.2f, posZ=%.2f, useSmoothTurn=%s, turnEnd=%s, turnReverse=%s, reverseWhenPosible=%s"):format(nameNum(vehicle), posX, posZ, tostring(useSmoothTurn and true or false), tostring(turnEnd and true or false), tostring(turnReverse and true or false), tostring(reverseWhenPosible and true or false)), 14);
	end;
end
...
function courseplay:oldTurn(self, dt) -- TODO: (Claus) Clean up all the unused variables that left out from the new turn system and delete the function
	--[[ TURN STAGES:
	0:	raise implements
