...
-- changed in 5.03.00044 
	...
	if id ~= nil and id ~= "" then
		...
		if #vehicle.Waypoints == 0 then
			...
			local wpDistMax = 30 --suggest reduction from 50 
				...
				if #crossingPoints[1] > 0 and #crossingPoints[2] > 0 then
					for _,wpNum1 in pairs(crossingPoints[1]) do
						...
						for _,wpNum2 in pairs(crossingPoints[2]) do
							...
							local angleTurn = 180 * math.atan2(wp2.cx-wp1.cx,wp2.cz-wp1.cz) / math.pi -- convert to degrees  
							--add direction change differences between original direction and turn direction and destination direction							
							local totalAngle=courseplay.utils.angleDifference(wp1.angle,angleTurn)+courseplay.utils.angleDifference(wp2.angle,angleTurn) 
							...
							if dist and dist ~= 0 and dist < wpDistMax then
								courseplay:debug(string.format('course1 wp %d angle %s course2 wp %d angle %s angleTurn %s, totalAngle %s', 
								wpNum1, tostring(wp1.angle), wpNum1, tostring(wp2.angle),tostring(angleTurn) , tostring(totalAngle)), 8);
								if totalAngle < smallestAngle then
