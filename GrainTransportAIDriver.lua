...
function GrainTransportAIDriver:checkLastWaypoint()
	...
	if self.ppc:getCurrentWaypointIx() == self.course:getNumberOfWaypoints() then
		...
		if self:getIsRunCounterActive() and self.runCounter >= self.vehicle.cp.settings.runCounterMax:get() then
...
function GrainTransportAIDriver:getIsRunCounterActive()
	return not self.vehicle.cp.settings.runCounterMax:is(RunCounterMaxSetting.RUN_COUNTER_OFF)
end
