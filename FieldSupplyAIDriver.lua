...
function FieldSupplyAIDriver:unloadingTriggerCallback(superFunc,triggerId, otherId, onEnter, onLeave, onStay, otherShapeId)
	...
	if courseplay:isAIDriverActive(rootVehicle) and rootVehicle.cp.driver.triggerHandler.validFillTypeUnloadingAugerWagon then 
        ...
        if object ~= nil and object ~= self and object:isa(Vehicle) then
			...
			objectTriggerHandler:enableTriggerSpeed(NetworkUtil.getObjectId(rootVehicle),object)
                ...
                if fillUnitIndex ~= nil then
                    ...
                    if dischargeNode ~= nil then
                        ...
                        if fillType and validFillUnitIndex then 
							...
							objectTriggerHandler:enableTriggerSpeed(NetworkUtil.getObjectId(rootVehicle),object,true)
						end
					end
				end
			elseif onLeave then
