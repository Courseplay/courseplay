function courseplay:handle_mode6(self, allowedToDrive, workSpeed, fillLevelPct, lx , lz )
	...
	for i=1, #(self.tippers) do
		...
		if workTool ~= nil and tool.attachedCutters == nil then
			...
			local fillType = tool.lastValidFillType
			local chopperIsStarting = false
			if fillType == 0 then
				chopperIsStarting = true
			end
			if workArea and not tool.isAIThreshing and self.cp.abortWork == nil and self.cp.turnStage == 0 then
				...
				if not specialTool then
					...
					if tool.capacity == 0 then
						if courseplay:isFoldable(workTool) and not tool.isTurnedOn and not isFolding and not isUnfolded then
							courseplay:debug(string.format('%s: unfold order (foldDir=%d)', nameNum(workTool), workTool.cp.realUnfoldDirection), 17);
							workTool:setFoldDirection(workTool.cp.realUnfoldDirection);
						end;
						if isUnfolded and not tool.isTurnedOn then
								--tool:startAIThreshing(); 
						...
						if tool.cp.isChopper and fillType ~= FruitUtil.FRUITTYPE_UNKNOWN and not chopperIsStarting then
							local targetTrailer = tool:findAutoAimTrailerToUnload(fillType);
							local trailer, trailerDistance = tool:findTrailerToUnload(fillType);
							if targetTrailer == nil or trailer == nil then
								chopperWaitForTrailer = true;
							end;
						end;
						
						if not chopperIsStarting and ((pipeState == 0 and self.cp.turnStage == 0) or chopperWaitForTrailer) then
						...
						if tankFillLevelPct >= 100 or tool.waitingForDischarge or (tool.cp.stopWhenUnloading and tool.pipeIsUnloading and tool.courseplayers and tool.courseplayers[1] ~= nil) or tool.stopForManualUnloader then
							...
							if tankFillLevelPct < 80 and (not tool.cp.stopWhenUnloading or (tool.cp.stopWhenUnloading and (tool.courseplayers == nil or tool.courseplayers[1] == nil))) then
								...
								if not weatherStop and not tool.isTurnedOn then
									tool:startThreshing();
								end;
							end;
			...
			if tool.cp.isCombine and tool.isTurnedOn and tool.fillLevel >= tool.capacity*0.8  or ((pipeState > 0 or courseplay:isAttachedCombine(workTool))and not courseplay:isSpecialChopper(workTool))then
				tool:setPipeState(2)
			...
			if tool.cp.waitingForTrailerToUnload then
				...
				elseif tool.cp.isChopper then
					...
					--if (tool.pipeParticleSystems and ((tool.pipeParticleSystems[ch] and tool.pipeParticleSystems[ch].isEmitting) or (tool.pipeParticleSystems[gr] and tool.pipeParticleSystems[gr].isEmitting))) or pipeState > 0 then FS15
						if fillType ~= FruitUtil.FRUITTYPE_UNKNOWN then
							local targetTrailer = tool:findAutoAimTrailerToUnload(fillType);
							local trailer, trailerDistance = tool:findTrailerToUnload(fillType);
