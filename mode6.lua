	...
	for i=1, #(vehicle.cp.workTools) do
		...
		if workTool ~= nil and tool.attachedCutters == nil then
          ...
          if vehicle.Waypoints[vehicle.cp.previousWaypointIndex].laneDir ~= nil then -- generated field course, no headland
            vehicle.cp.abortWork = vehicle.cp.previousWaypointIndex - math.floor(10 / CpManager.courseStepForPoints + 0.5) - 2; -- optimized reduced Waypoints: we need less backup before restarting work
          end;
