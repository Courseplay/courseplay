	...
	if self.isMouseWheelArea then
		...
		if pg == courseplay.hud.PAGE_CP_CONTROL then
			...
			elseif fn == 'changeRunNumber' then
 				canScrollUp = vehicle.cp.runNumber < 11 and (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT or vehicle.cp.mode == courseplay.MODE_LIQUIDMANURE_TRANSPORT) and vehicle.cp.canDrive  and not vehicle.cp.runReset and #vehicle.cp.easyFillTypeList > 0;
 				local canChange = true
				if (vehicle.cp.fillTrigger or vehicle.cp.isInFilltrigger) or (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT and vehicle.cp.waypointIndex == 1 and not vehicle.cp.runCounter ~= 0) then
					canChange = vehicle.cp.runNumber - vehicle.cp.runCounter > 1
				end
 				canScrollDown = vehicle.cp.runNumber > vehicle.cp.runCounter and vehicle.cp.runNumber > 1 and (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT or vehicle.cp.mode == courseplay.MODE_LIQUIDMANURE_TRANSPORT)  and vehicle.cp.canDrive and canChange  and not vehicle.cp.runReset and #vehicle.cp.easyFillTypeList > 0;
			end;
				...
				elseif fn == 'changeRunNumber' then
 					if prm < 0 then
						...
						if (vehicle.cp.fillTrigger or vehicle.cp.isInFilltrigger) or (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT and vehicle.cp.waypointIndex == 1 and not vehicle.cp.runCounter ~= 0) then
 						...
 						show = vehicle.cp.runNumber > vehicle.cp.runCounter and vehicle.cp.runNumber > 1 and (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT or vehicle.cp.mode == courseplay.MODE_LIQUIDMANURE_TRANSPORT) and vehicle.cp.canDrive and canChange and not vehicle.cp.runReset and #vehicle.cp.easyFillTypeList > 0;
 					elseif prm > 0 then
 						show = vehicle.cp.runNumber < 11 and (vehicle.cp.mode == courseplay.MODE_GRAIN_TRANSPORT or vehicle.cp.mode == courseplay.MODE_LIQUIDMANURE_TRANSPORT) and vehicle.cp.canDrive and not vehicle.cp.runReset and #vehicle.cp.easyFillTypeList > 0;
 					end;
				end;
