...
function courseplay.hud:loadPage(vehicle, page)
	...
	elseif page == 6 then
		...
		if not (vehicle.cp.mode == 3 or vehicle.cp.mode == 4 or vehicle.cp.mode == 6 or vehicle.cp.mode == 7) then
			...
			vehicle.cp.hud.content.pages[6][5][2].text = string.format("%.1f sec", vehicle.cp.waitTime);
		end;
	...
	elseif page == 7 then
		if vehicle.cp.mode == 3 or vehicle.cp.mode == 4 or vehicle.cp.mode == 6 or vehicle.cp.mode == 7 then
			--Lane offset
