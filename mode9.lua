...
function courseplay:handle_mode9(vehicle, fillLevelPct, allowedToDrive, lx, lz, dt)
    ...
    if vehicle.cp.totalCapacity == nil or vehicle.cp.totalCapacity == 0 then
        -- NOTE: query here instead of getCanUseAiMode() as tipperCapacity doesn't exist until drive() has been run
    ...
    if vehicle.cp.waypointIndex == 1 and vehicle.cp.shovelState ~= 6 then
        -- backup for missed approach
                    ...
                    if vehicle.cp.BunkerSiloMap ~= nil then
                        ...
                        for lineIndex, line in pairs(vehicle.cp.BunkerSiloMap) do
                            ...
                            for column, fillUnit in pairs(line) do
                                ...
                                if column == #line and mostFillLevelAtLine > math.min(vehicle.cp.shovel.cp.capacity * 0.1, 200) then
                                    -- avoid little quantities
    ...
    elseif vehicle.cp.shovelState == 2 then
        ...
        if vehicle.cp.mode9TargetSilo and vehicle.cp.BunkerSiloMap and vehicle.cp.actualTarget then
            ...
            local _, _, backUpZ = worldToLocal(vehicle.cp.DirectionNode, cx, targetUnit.y, cz);
            -- its the savety switch in case I miss the point
            local distanceToTarget = courseplay:distance(nx, nz, cx, cz)
            -- distance from shovel to target
...
function courseplay:getCurrentMovingToolsPosition(vehicle, movingTools, secondary, fixIndex)
    -- NOTE: still needed for saveShovelPosition()
    if movingTools == nil then
        print(nameNum(vehicle) .. ': courseplay:getCurrentMovingToolsPosition() return nil');
        return nil;
    end;

    local rotAxis, transAxis = 1, 3;
    -- 1 = x, 3 = z;
...
function courseplay:checkAndSetMovingToolsPosition(vehicle, movingTools, secondaryMovingTools, targetPositions, dt, fixIndex)
    ...
    for i = 1, #targetRotations do
        ...
        if courseplay:round(curRot, 4) ~= courseplay:round(targetRot, 4) or courseplay:round(curTrans, 3) ~= courseplay:round(targetTrans, 3) then
            ...
            -- TRANSLATION
            if mt.transSpeed ~= nil then
                -- only change values if transSpeed actually exists
                ...
                if mt.node and mt.transMin and mt.transMax and transDir and transDir ~= 0 then
                    local transChange = math.min(mt.transSpeed, 0.001) * dt;
                    -- maximum: 1mm/ms (1m/s)
...
function courseplay:getBunkerSiloFilling(map)
    ...
    for i, fillLevel in pairs(FillLevels) do
        if fillLevel > maxValues.fillLevel and string.upper(FillUtil.fillTypeIndexToDesc[i].name) ~= "TARP" then
            -- ignore tarp (stuff under the plane))
